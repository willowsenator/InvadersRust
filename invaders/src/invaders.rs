...
impl Invaders {
    ...
    pub fn update(&mut self, delta: Duration) -> bool {
        ...
        if self.move_timer.ready {
            ...
            if self.direction == -1 {
                let min_x = self.army.iter().map(|invader| (invader.x)).min()
                    .unwrap_or(0);
                if min_x == 0 {
                    self.direction = 1;
                    downwards = true;
                ...
                let max_x = self.army.iter().map(|invader|(invader.x)).max()
                    .unwrap_or(0);
                if max_x == NUM_COLS - 1 {
                    self.direction = -1;
                    downwards = true;
    ...
    pub fn kill_invader_at(&mut self, x:usize, y:usize) -> bool {
        if let Some(idx) = self.army.iter().position(|invader| (invader.x == x)
            && (invader.y == y)){
